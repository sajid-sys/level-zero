      ...
      <div className="flex items-center justify-between pb-3 border-b border-slate-100 mb-4">
        <div>
          <h2 className="text-sm font-semibold text-slate-900 uppercase tracking-wider">
            Battery Action Timeline
          </h2>
          <p className="text-xs text-slate-500 font-medium">Hourly Charge / Discharge Activity</p>
        </div>
        <div className="flex items-center gap-3 text-xs">
          <span className="flex items-center gap-1">
            <span className="w-2.5 h-2.5 rounded-sm bg-emerald-500" /> Charge
          </span>
          <span className="flex items-center gap-1">
            <span className="w-2.5 h-2.5 rounded-sm bg-purple-500" /> Discharge
          </span>
          <span className="flex items-center gap-1">
            <span className="w-2.5 h-2.5 rounded-sm bg-slate-200" /> Idle
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-6 sm:grid-cols-12 md:grid-cols-24 gap-1 text-center font-mono">
        {hourlyPlan.map((entry) => {
          ...
          return (
            <div
              key={entry.hour}
              className={`p-1.5 rounded border text-[11px] flex flex-col justify-between min-h-[52px] ${bg}`}
